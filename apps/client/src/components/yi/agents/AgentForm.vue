      ...
      <div class="agent-form-left">
        ...
        <div class="avatar-actions">
          <label class="avatar-upload-btn">
            ...
            {{ t('agents.form.uploadAvatar') }}
          </label>
          <button v-if="form.avatar" class="avatar-remove-btn" @click="removeAvatar">
            {{ t('agents.form.removeAvatar') }}
          </button>
        </div>
      </div>
      <div class="agent-form-right">
        <n-form-item :label="t('agents.form.name')" label-placement="top" size="small">
          <n-input v-model:value="form.name" :placeholder="t('agents.form.namePlaceholder')" />
        </n-form-item>
        <n-form-item :label="t('agents.form.description')" label-placement="top" size="small">
          <n-input v-model:value="form.description" type="textarea" :rows="2" />
        </n-form-item>
        <div class="agent-form-row">
          <n-form-item :label="t('agents.form.mode')" label-placement="top" size="small" class="flex-1">
            <n-select v-model:value="form.mode" :options="modeOptions" />
          </n-form-item>
          <n-form-item :label="t('agents.form.maxSteps')" label-placement="top" size="small" class="flex-1">
            <n-slider v-model:value="form.maxSteps" :min="1" :max="50" :step="1" />
          </n-form-item>
        </div>
        <n-form-item :label="t('agents.form.enabled')" label-placement="top" size="small">
      ...
      <div class="memory-section">
        <div class="section-header">
          <div class="section-title-row">
            ...
            <span class="section-title">{{ t('agents.form.tabs.soul') }}</span>
          </div>
        ...
        <div v-if="editingSection !== 'soul'" class="section-body">
          <p v-if="!form.soul" class="empty-text">{{ t('agents.form.soulPlaceholder') }}</p>
          <div v-else class="content-text">{{ form.soul }}</div>
        </div>
        <div v-else class="section-edit">
          <textarea v-model="editContent" class="edit-textarea" :placeholder="t('agents.form.soulPlaceholder')" spellcheck="false" />
      ...
      <div class="memory-section">
        <div class="section-header">
          <div class="section-title-row">
            ...
            <span class="section-title">{{ t('agents.form.tabs.user') }}</span>
          </div>
        ...
        <div v-if="editingSection !== 'user'" class="section-body">
          <p v-if="!form.userProfile" class="empty-text">{{ t('agents.form.userProfilePlaceholder') }}</p>
          <div v-else class="content-text">{{ form.userProfile }}</div>
        </div>
        <div v-else class="section-edit">
          <textarea v-model="editContent" class="edit-textarea" :placeholder="t('agents.form.userProfilePlaceholder')" spellcheck="false" />
          <div class="edit-actions">
            <NButton size="small" @click="cancelEdit">{{ t('common.cancel') }}</NButton>
            <NButton size="small" type="primary" @click="saveEdit">{{ t('common.save') }}</NButton>
          </div>
      ...
      <div class="memory-section">
        <div class="section-header">
          <div class="section-title-row">
            ...
            <span class="section-title">{{ t('agents.form.tabs.memory') }}</span>
          </div>
        ...
        <div v-if="editingSection !== 'memory'" class="section-body">
          <p v-if="!form.memory?.enabled" class="empty-text">{{ t('agents.form.memoryDisabledHint') }}</p>
          <p v-else-if="!form.memoryContent" class="empty-text">{{ t('agents.form.memoryPlaceholder') }}</p>
          <div v-else class="content-text">{{ form.memoryContent }}</div>
        </div>
        <div v-else class="section-edit">
          <textarea v-model="editContent" class="edit-textarea" :placeholder="t('agents.form.memoryPlaceholder')" spellcheck="false" />
          <div class="edit-actions">
            <NButton size="small" @click="cancelEdit">{{ t('common.cancel') }}</NButton>
            <NButton size="small" type="primary" @click="saveEdit">{{ t('common.save') }}</NButton>
          </div>
...
.agent-form-right { flex: 1; min-width: 0; }
.agent-form-row { display: flex; gap: 16px; }
.flex-1 { flex: 1; }

/* Sections - same style as memory page */
.agent-form-sections {
  flex: 1;
  display: flex;
  flex-direction: column;
