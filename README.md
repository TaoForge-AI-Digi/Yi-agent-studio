...
<p align="center">
  <em>开源的桌面智能体——你说意图,它落子执行;你沉淀知识,它越用越聪明;你分享智慧,它帮你变现。</em>
</p>

弈(读作 *yì*,古称围棋)是一个面向知识工作者的开源桌面 Agent。它把"用户布势、Agent 落子"的围棋范式搬进 GUI:你说出意图,弈自己规划、分步执行、沉淀棋谱,所有执行记录可复用、可分享、可在弈林市场流通。
...
| 已有玩家 | 占据什么 | 弈的差位 |
|---|---|---|
| OpenCode / Goose | 开源编码 Agent,CLI/TUI,插件机制很弱 | 同样开源 + **GUI 优先** + 面向知识工作者,不绑死编码场景 |
| Codex / Claude Code / Cursor | 强 Agent,但闭源、绑模型 | **模型自由** + **开源** + 通用桌面 |
| Hermes / OpenClaw | 桌面快捷指令,角色系统太重 | 弈更轻:**意图即指令**,有规划、有棋谱、有棋经,不堆角色 |
| Obsidian / Notion / Logseq | 本地知识库,知识是"死"的 | 弈的棋经是 **Agent 的外挂大脑**——可调用、可组合、可在弈林变现 |
